...
class Pix2Pix():
    ...
    def train(self, epochs, batch_size=1, sample_interval=50,nsample=10):
        start_time = datetime.datetime.now()
        # Adversarial loss ground truths
        valid = np.ones((batch_size,) + self.disc_patch)
        fake = np.zeros((batch_size,) + self.disc_patch)
        for epoch in range(epochs):
            for batch_i, (imgs_A, imgs_B) in range(nsample):
                # ---------------------
                #  Train Discriminator
                # ---------------------
...
gan = Pix2Pix()
gan.train(epochs=200, batch_size=1, sample_interval=50,nsample=200)
